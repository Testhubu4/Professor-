...
async def pm_next_page(bot, query):
    ...
    btn.insert(0, [InlineKeyboardButton("‼️ Join ᴛᴏ BACKUP ‼️", url= "https://t.me/hub4u_updates")])
...
async def pm_spoll_tester(bot, query):
    ...
    await query.answer('Cʜᴇᴄᴋɪɴɢ Fᴏʀ Webseries Iɴ Dᴀᴛᴀʙᴀsᴇ...')
    files, offset, total_results = await get_search_results(movie, offset=0, filter=True)
    if files:
        k = (movie, files, offset, total_results)
        await pm_AutoFilter(bot, query, k)
    else:
        k = await query.message.edit('Tʜɪs Webseries Nᴏᴛ Fᴏᴜɴᴅ Iɴ Dᴀᴛᴀʙᴀsᴇ')
...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏ Webseries Iɴ Tʜᴀᴛ Nᴀᴍᴇ", quote=True)
        await asyncio.sleep(10)
